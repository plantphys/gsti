...
- Shawn, what do you think of the requirements? I am not as strict as for ESS dive as I dont ask the RHs column. I could also remove the Patm column (assumed to be 101 in the fitting). We should try to be flexible so people are not scared if the requirements are to hard.
...
Need your help here Shawn. I saw that you smooth the leaf reflectance when you do the processing, does it matter or not? Do you have a reference for the processing? What do we do if people give us the raw instrument output?

The full range processed reflectance data should be added to the datasets. 
We use the corrected reflectance expressed in the scale 0 100 with 1 nm resolution, i.e, an interpolated reflectance corrected for the jump between sensors.
...
- Shawn, this data frame is used then for the PLSR. We could decide to add more columns, for example the column sigma or Tleaf from the ACi curves. We should discuss that so we dont have to redo some of the processing for the datasets and then hold tight with what we decided. The more this project will grow, the more work we will have if we change the requirements. So we have to think smart right now ;)
