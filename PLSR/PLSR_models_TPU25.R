...
hist(Database$TPU25)
hist(sqrt(Database$TPU25)) 

## Removing Na values to avoid any issue
Database = Database[!is.na(Database$TPU25),]

Database$sqrt_TPU25=sqrt(Database$TPU25)
...
## Only keeping spectra collected on the same leaves as gas exchange
Database=Database[Database$Spectra_trait_pairing=="Same",]
## Keeping only relevant columns to reduce the database size
Database = Database[,c("SampleID","Dataset_name","TPU25","StdError_TPU25","sqrt_TPU25","Spectra")]


## Using a LOO PLSR model to identify outliers. ~1% of the values are removed
## This process takes 6 minutes
test_LOO = plsr(sqrt_TPU25~ Spectra,ncomp = 20, data = Database, validation = "LOO")
res_LOO = test_LOO$validation$pred[,1,20]-Database$sqrt_TPU25
hist(res_LOO)
...
hist(Database$StdError_TPU25/Database$TPU25)
Database=Database[Database$StdError_TPU25/Database$TPU25<0.15|is.na(Database$StdError_TPU25),]
...
save(file="PLSR_model_TPU25.Rdata",plsr_model,training,validation)
