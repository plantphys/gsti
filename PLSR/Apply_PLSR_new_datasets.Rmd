...
This tutorial shows how to apply pre-trained Partial Least Squares Regression (PLSR) models developed using the GSTI project to predict photosynthetic traits from leaf reflectance spectra. The models for `Vcmax25`, `Jmax25`, `TPU25`, and `Rdark25` are available here:
...
The plsr_model object is a list with several components. A key component for the prediction is coefs: A matrix containing 1000 sets of coefficients from the 1000 bootstrapped PLSR models. Row 1 corresponds to the intercepts for the 1000 models and rows 2-2102 correspond to the coefficients associated with the reflectance wavelengths from 400 to 2500 nm.
