...
jpeg("Figure2.jpeg", height=120, width=120,units = 'mm',res=300)
...
geom_point()+coord_cartesian(xlim=c(0,250),ylim=c(0,250))+
  geom_abline(slope=1) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  ylab(expression(Observed~italic(V)[cmax25]))+xlab(expression(Predicted~italic(V)[cmax25]))+annotate(x=0,y=250,label=paste("R² =",round(plsr_model$R2,2)),"text",hjust=0,vjust=1)+annotate(x=0,y=235,label=paste("RMSE =",round(plsr_model$RMSE,1)),"text",hjust=0,vjust=1)
dev.off()

jpeg("Figure2_dataset.jpeg", height=120, width=180,units = 'mm',res=300)
