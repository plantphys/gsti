...
# Checking if there are duplicated CO2r points in the ACi curves and only keeping the first one
Remove_Aci_dupplicate<-function(Aci_curve){
  ...
  ## We now remove the duplicates in the middle of the ACi curves, except the last
...
#Remove=ave(round(curated_data$CO2r,-1),curated_data$SampleID,FUN=duplicated)
...
QC_table=cbind.data.frame(SampleID_num=c(56,119,119,119,210,213,258),
                          Record=c(11,10,11,12,9,9,1)) 
# Here I flag the bad curves by writing down the SampleID_num of the bad curves.
ls_bad_curve=c(19,79:82,87,96,138,140:142,151,163,164,174,179,195,196,198:208,211,216,217,221,226)
ls_bad_curve=c(ls_bad_curve,12,132,139,147,166,172,176,185,197,209,228) # I removed these curves based on the fittings
