...
curated_data=curated_data[-which(curated_data$SampleID=='BNL10446'&curated_data$Record>18),]
...
OK_table=cbind.data.frame(SampleID_num=c(2,76,84,94,95,114,134,150),Record=c(18,17,17,17,17,17,37,19)) ## 
curated_data[paste(curated_data$SampleID_num,curated_data$Record)%in%paste(OK_table$SampleID_num,OK_table$Record),'QC']='ok'
...
QC_table=cbind.data.frame(SampleID_num=c(1,2,4,5,7,8,10,11,13,21,21,33,35,35,39,44,47,49,51,53,56,61,63,75,76,84,91,94,94,106,116,142,146,146,146,156,157,161,171,173,173,180,190,198,202,209,241),
                          Record=c(12,11,51,87,106,3,79,52,42,3,18,17,38,39,25,58,62,2,12,53,36,69,42,10,10,15,1,1,10,1,10,75,95,94,97,4,19,39,64,22,31,90,34,13,67,84,92)) ## 
