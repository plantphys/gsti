...
path=here()
source(paste(path,'/R/Johnson_Berry_Aci_model.R',sep=''))
...
#'
#' @examples
f.fit_Aci_JB<-function(measures,param,VcmaxRef=60, VqmaxRef=120, RdRef = 2, TpRef= 5){
 
## Fitting of the Aci curve using Ac and Aj limitation
   pdf(file = '2_JB_ACi_fitting_Ac_Aj.pdf')
  result_Ac_Aj=by(data = measures,INDICES = list(measures$SampleID_num),
            FUN = function(x){f.fitting_JB(measures = x,Start = list(VqmaxRef=VqmaxRef,RdRef=RdRef,VcmaxRef=VcmaxRef),
                                        param=f.make.param_JB(TpRef=9999),id.name = 'SampleID_num')})
  dev.off()
  
## Fitting of the Aci curve using only the Ac limitation
  pdf(file = '2_JB_ACi_fitting_Ac.pdf')
  result_Ac=by(data = measures,INDICES = list(measures$SampleID_num),
               FUN = function(x){f.fitting_JB(measures = x,Start = list(RdRef=RdRef,VcmaxRef=VcmaxRef),
                                           param=f.make.param_JB(TpRef=9999,VqmaxRef=9999),id.name = 'SampleID_num')})
  dev.off()
  
##Fitting of the curve using the Ac, Aj and Ap limitations
  pdf(file = '2_JB_ACi_fitting_Ac_Aj_Ap.pdf')
  result_Ac_Aj_Ap=by(data = measures,INDICES = list(measures$SampleID_num),
               FUN = function(x){
                 ...
                 f.fitting_JB(measures = x,Start = Start,
                           param=f.make.param_JB(),id.name = 'SampleID_num',modify.init =modify.init )})
  dev.off()
...
## Finding the best model (Ac or Ac_Aj or Ac_Aj_Ap according to the BIC criterion)
  ...
  Bilan_JB=res_nlf_Ac_Aj
  Bilan_JB[which(res_nlf_Ac$BIC<res_nlf_Ac_Aj$BIC),]=res_nlf_Ac[which(res_nlf_Ac$BIC<res_nlf_Ac_Aj$BIC),]
  Bilan_JB[which(res_nlf_Ac_Aj_Ap$BIC<Bilan_JB$BIC),]=res_nlf_Ac_Aj_Ap[which(res_nlf_Ac_Aj_Ap$BIC<Bilan_JB$BIC),]
  colnames(Bilan_JB)=c("sigma","VcmaxRef","VqmaxRef","TpRef","RdRef","BIC","Tleaf","SampleID_num","model") 
  
### Creating a pdf with the best model for each curve
  
  pdf(file = '2_JB_ACi_fitting_best_model.pdf')
  by(data = measures,INDICES = list(measures$SampleID_num),
     FUN = function(x){
       param=Bilan_JB[Bilan_JB$SampleID_num==unique(x$SampleID_num),c('VcmaxRef','VqmaxRef','RdRef','TpRef')]
       param_leg=param
       param[is.na(param)]=9999
       param_curve=f.make.param_JB(VcmaxRef = param[['VcmaxRef']],VqmaxRef=param[['VqmaxRef']],RdRef=param[['RdRef']],TpRef=param[['TpRef']])
       f.plot_JB(measures=x,list_legend = as.list(param_leg),param = param_curve,name =unique(x$SampleID_num) )
     }
  )
  dev.off()
  colnames(Bilan_JB)=paste('JB',colnames(Bilan_JB),sep='_')
  return(Bilan_JB)
}
